...
class TradingBot:
    ...
    def __init__(self, symbol: str, start_date: str, end_date: str, budget: float=5000.0):
    ...
    def fetch_data(self) -> bool:
        ...
        if self.data is None or self.data.empty:
            console.print("[bold red]No data fetched. Check symbol or date range.[/bold red]")
            return False
    ...
    def calculate_moving_averages(self) -> bool:
        ...
        if self.data is None or self.data.empty:
            Console().print("[bold red]Cannot calculate moving averages, no data available.[/bold red]")
            return False
    ...
    def display_summary(self):
        console = Console()
        profit_loss = self.budget - self.initial_budget
        percentage_return = (profit_loss / self.initial_budget) * 100
        profit_style = "bold green" if profit_loss >= 0 else "bold red"
        profit_sign = "+" if profit_loss >= 0 else ""
        summary_text = Text(f"Initial Budget: ${self.initial_budget:,.2f}\n", justify="left")
        summary_text.append(f"Final Budget:   ${self.budget:,.2f}\n")
        summary_text.append(f"Profit/Loss:    {profit_sign}${profit_loss:,.2f} ({profit_sign}{percentage_return:.2f}%)", style=profit_style)

        console.print(Panel(summary_text, title="[bold cyan]Trading Strategy Evaluation[/bold cyan]", expand=False, border_style="cyan"))

        if self.trades:
            trades_table = Table(title="[bold]Trade Log[/bold]", show_header=True, header_style="bold magenta")
            trades_table.add_column("Date", style="dim", width=12, justify="center")
            trades_table.add_column("Action", justify="center")
            trades_table.add_column("Shares", justify="right")
            trades_table.add_column("Price", justify="right")
            trades_table.add_column("Trade Value", justify="right")

            for trade in self.trades:
                action_style = "green" if "BUY" in trade['action'] else "red"
            ...
            console.print("[yellow]No trades were executed during this period.[/yellow]")

    def plot_results(self):
        if self.data is None or self.data.empty:
            Console().print("[bold red]Cannot plot results, no data available.[/bold red]")
            return
        plt.style.use('seaborn-v0_8-darkgrid')
        fig, ax = plt.subplots(figsize=(15, 8))
        ax.plot(self.data.index, self.data['Close'], label='Close Price', color='skyblue', alpha=0.8, linewidth=1.5)
        ax.plot(self.data.index, self.data['MA50'], label='50-Day MA', color='orange', linestyle='--', linewidth=1.2)
        ax.plot(self.data.index, self.data['MA200'], label='200-Day MA', color='purple', linestyle='--', linewidth=1.2)

        if self.trades:
            buy_dates = [trade['date'] for trade in self.trades if 'BUY' in trade['action']]
            buy_prices = [trade['price'] for trade in self.trades if 'BUY' in trade['action']]
            sell_dates = [trade['date'] for trade in self.trades if 'SELL' in trade['action']]
            sell_prices = [trade['price'] for trade in self.trades if 'SELL' in trade['action']]

            ax.scatter(buy_dates, buy_prices, marker='^', color='green', s=150, label='Buy Signal', zorder=5, edgecolors='black')
            ax.scatter(sell_dates, sell_prices, marker='v', color='red', s=150, label='Sell Signal', zorder=5, edgecolors='black')

        ax.set_title(f'Golden Cross Trading Strategy for {self.symbol}', fontsize=18)
